...
PREFIX="/usr/local/php7022"
DOWNLOAD_PHP_URL="https://raw.githubusercontent.com/viagram/PHP_Install/master/7.0/7022"
ZEND_ARCH="x86"
LIB="lib"
...
[Unit]
Description=php7022 fpm server
After=network.target remote-fs.target nss-lookup.target

[Service]
Type=forking
PIDFile=/var/run/php-fpm.pid
ExecStart=/usr/local/php7022/sbin/php-fpm --daemonize --fpm-config /usr/local/php7022/etc/php-fpm.conf --pid /var/run/php-fpm.pid
Restart=on-failure
PrivateTmp=true
...
function install_php(){
    wget -O php-7.0.22.tar.gz -c $DOWNLOAD_PHP_URL/php-7.0.22.tar.gz
    tar zxf php-7.0.22.tar.gz
    cd php-7.0.22

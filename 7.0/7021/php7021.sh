...
PREFIX="/usr/local/php7021"
DOWNLOAD_PHP_URL="https://raw.githubusercontent.com/viagram/PHP_Install/master/7.0/7021"
ZEND_ARCH="x86"
LIB="lib"
...
[Unit]
Description=php7021 fpm server
After=network.target remote-fs.target nss-lookup.target

[Service]
Type=forking
PIDFile=/var/run/php-fpm.pid
ExecStart=/usr/local/php7021/sbin/php-fpm --daemonize --fpm-config /usr/local/php7021/etc/php-fpm.conf --pid /var/run/php-fpm.pid
Restart=on-failure
PrivateTmp=true
...
function install_php(){
    wget -O php-7.0.21.tar.gz -c $DOWNLOAD_PHP_URL/php-7.0.21.tar.gz
    tar zxf php-7.0.21.tar.gz
    cd php-7.0.21
...
function install_ioncube(){
    ...
    unzip -o ioncube-$ZEND_ARCH-7.0.zip
    phpext_dir=$($PREFIX/bin/php-config --extension-dir)
    \cp ioncube_loader_lin_7.0.so $phpext_dir/ioncube_loader_lin_7.0.so

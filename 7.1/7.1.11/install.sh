...
DOWNLOAD_PHP_URL="https://raw.githubusercontent.com/viagram/PHP_Install/master/7.1/7.1.11"
ZEND_ARCH="x86"
LIB="lib"
...
function install_php(){
    wget -O php-7.1.11.tar.gz -c $DOWNLOAD_PHP_URL/php-7.1.11.tar.gz
    if [[ $? -ne 0 ]]; then
        echo -e "\033[31m下载PHP源码包失败.\033[0m"
        exit 1
    fi
    ...
    if [ ! -f $PREFIX/lib/php.ini ]; then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "\033[31m下载php.ini失败.\033[0m"
            exit 1
        fi
    fi

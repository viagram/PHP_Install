...
PREFIX="/usr/local/php-7.1.10"
DOWNLOAD_PHP_URL="https://raw.githubusercontent.com/viagram/PHP_Install/master/7.1/7.1.10"
ZEND_ARCH="x86"
LIB="lib"
...
[Unit]
Description=php-7.1.10 fpm server
After=network.target remote-fs.target nss-lookup.target

[Service]
Type=forking
PIDFile=/var/run/php-fpm.pid
ExecStart=/usr/local/php-7.1.10/sbin/php-fpm --daemonize --fpm-config /usr/local/php-7.1.10/etc/php-fpm.conf --pid /var/run/php-fpm.pid
Restart=on-failure
PrivateTmp=true
...
function install_php(){
    wget -O php-7.1.10.tar.gz -c $DOWNLOAD_PHP_URL/php-7.1.10.tar.gz
...
function install_ioncube(){
    wget -O ioncube-$ZEND_ARCH-7.1.zip -c $DOWNLOAD_PHP_URL/ioncube-$ZEND_ARCH-7.1.zip
    unzip -o ioncube-$ZEND_ARCH-7.1.zip
    phpext_dir=$($PREFIX/bin/php-config --extension-dir)
    \cp ioncube_loader_lin_7.1.so $phpext_dir/ioncube_loader_lin_7.1.so
    chmod 755 $phpext_dir/ioncube_loader_lin_7.1.so
}

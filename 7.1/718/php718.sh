...
PREFIX="/usr/local/php718"
DOWNLOAD_PHP_URL="https://raw.githubusercontent.com/viagram/PHP_Install/master/7.1/718"
ZEND_ARCH="x86"
LIB="lib"
...
[Unit]
Description=php718 fpm server
After=network.target remote-fs.target nss-lookup.target

[Service]
Type=forking
PIDFile=/var/run/php-fpm.pid
ExecStart=/usr/local/php718/sbin/php-fpm --daemonize --fpm-config /usr/local/php718/etc/php-fpm.conf --pid /var/run/php-fpm.pid
Restart=on-failure
PrivateTmp=true
...
function install_php(){
    wget -O php-7.1.8.tar.gz -c $DOWNLOAD_PHP_URL/php-7.1.8.tar.gz
